...
- [x] 【完成】ConfigList 全模式支持：补全 `fixed` / `column` 表单与字段映射 UI，确保与 doc 中表头/列映射要求一致。（Owner: AI Assistant，2025-11-10）
...
- [x] 【完成】Carbone 标记无需配置声明：梳理存量逻辑、扩展 skip 绑定并串联数据库与前端流程，让公式字段可声明无需配置。（Owner: Codex，2025-11-16）
