# AGENT.md - 国富 Excel 工具开发指南

> **面向 AI 代理的项目规范文档**
> 本文档为 AI 编程助手提供项目架构、技术栈和开发最佳实践的全面指南。
...
├── src/
│   ├── main/              # 主进程（Node.js 环境）
│   │   └── index.ts       # 入口：创建窗口、IPC 通信、文件系统操作
...
├── tsconfig.node.json     # 主进程和预加载 TS 配置
...
- **职责**：
  ...
  - 调用 exceljs 解析数据
  - 子进程管理（通过 execa）
- **环境**：Node.js（可访问所有 Node API 和 Electron 主进程 API）
...
- **环境**：Chromium 浏览器环境（**不能**直接访问 Node API）
- **构建目标**：ESM
...
export default defineConfig({
  ...
  main: {
    plugins: [
      // 自动外部化 package.json 中的 dependencies
...
export async function loadContract(contractId: string): Promise<ReportContract> {
  ...
  try {
    ...
    throw new Error(`无法加载契约 ${contractId}: ${error.message}`)
  }
}
...
3. **自定义组件库**
   创建 `src/renderer/src/components/ui/` 目录存放通用组件：
   - `Button.vue`
   - `Input.vue`
   - `Select.vue`
   - `Modal.vue`
...
// 定义表单模式
const contractSchema = z.object({
  name: z.string().min(1, '契约名称不能为空').max(100, '名称过长'),
  ...
  <form @submit.prevent="handleSubmit" class="space-y-4">
    <div>
      <label for="name" class="block text-sm font-medium text-gray-700">
        契约名称 <span class="text-red-500">*</span>
      </label>
      <input
        id="name"
        v-model="form.name"
...
export async function renderReport(options: CarboneRenderOptions): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    carbone.render(templatePath, data, (err, result) => {
      if (err) {
        reject(new Error(`Carbone 渲染失败: ${err.message}`))
        return
      }
...
export async function parseTemplateMarks(templatePath: string): Promise<{
  ...
  // 注意：实际实现需要使用 exceljs 读取 Excel 单元格内容
  // 这里简化展示逻辑
...
productName: 国富Excel工具
copyright: Copyright © 2025 国富公司
...
本文档为 AI 代理提供了全面的项目指南。在开发过程中：

1. **遵循约定**：保持项目结构和命名规范的一致性
2. **安全第一**：始终验证 IPC 输入，使用 `contextIsolation`
3. **类型安全**：充分利用 TypeScript，避免 `any`
4. **性能考量**：避免主进程阻塞，优化 IPC 通信
5. **用户体验**：提供清晰的错误提示和加载状态

**最后提醒**：本项目的核心哲学是"模板驱动配置"，所有功能设计都应围绕此原则，确保用户体验的一致性和简洁性。
